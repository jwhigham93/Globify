/**
 * Shared supply-chain domain types, used by both apps/Globify (v1) and
 * apps/globify-tiles (v2). Extracted from
 * apps/Globify/src/components/Globe/types.ts (task 5 of
 * openspec/changes/tiles-globe-v2-init/) — that file now re-exports these
 * for backward compatibility with v1's existing imports.
 *
 * Rendering-specific types (RoutePathSegment, GlobeState,
 * GlobeVisualizationProps, ViewMode) stayed in the app — they're tied to
 * three-globe/react-globe.gl conventions and the GlobeVisualization
 * component's own props, not to the domain data itself.
 */
...
/**
 * Arc visualization data — a source/destination pair with styling. Shape
 * originated from three-globe's arcsData conventions, but is generic enough
 * (lat/lng endpoints + color/width/label) to work as a renderer-agnostic
 * data shape for any renderer that draws a line/arc between two points.
 */
...
/**
 * A data point visualized on the globe — a renderer-agnostic lat/lng +
 * styling shape (originated from react-globe.gl's pointsData conventions).
 */
